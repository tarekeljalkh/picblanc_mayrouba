...
<div class="invoice-preview center">
    ...
    <div class="invoice-preview-card">
                ...
                <tbody>
                    ...
                    @foreach ($invoice->customItems as $customItem)
                        <tr>
                            ...
                            <td class="text-right">${{ number_format($customItem->price * $customItem->quantity, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach ($invoice->returnDetails as $return)
                            ...
                            @endphp
                            <tr>
                                <td>{{ $return->invoiceItem->product->name ?? ($return->additionalItem->product->name ?? $return->customItem->name) }}</td>
        ...
        <div class="no-break">
            <table>
                <tbody>
                    <tr><td>Base Total:</td><td class="text-right">${{ number_format($totals['subtotalForDiscount'], 2) }}</td></tr>
                    <tr><td>Discount:</td><td class="text-right">-${{ number_format($totals['discountAmount'], 2) }}</td></tr>
                    <tr><td>Add Items Cost:</td><td class="text-right">${{ number_format($totals['additionalItemsCost'], 2) }}</td></tr>
                    <tr><td>Refund:</td><td class="text-right">-${{ number_format($totals['refundForUnusedDays'], 2) }}</td></tr>
                    <tr><td>Deposit:</td><td class="text-right">${{ number_format($invoice->deposit, 2) }}</td></tr>
                    <tr><td><strong>Final Total:</strong></td><td class="text-right"><strong>${{ number_format($totals['finalTotal'], 2) }}</strong></td></tr>
                    <tr><td>Paid:</td><td class="text-right">${{ number_format($invoice->paid_amount, 2) }}</td></tr>
                    <tr><td><strong style="color:red;">Balance:</strong></td><td class="text-right" style="color:red;"><strong>${{ number_format($totals['balanceDue'], 2) }}</strong></td></tr>
                </tbody>
            </table>
        </div>
